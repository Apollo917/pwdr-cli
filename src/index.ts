...
export const run: Run = async ({ phrase, keys, length }) => {
  ...
  const phraseBuffer = textEncoder.encode(phrase);

  const pwdBuffers = await Promise.all(
    keys.map((key) => generatePwd(phraseBuffer, textEncoder.encode(key.trim()), { length })),
  );
  pwdBuffers.forEach((pwdBuffer, i) => {
    output.push(`${OUTPUT_KEY} ${keys[i]}`);
    output.push(`${OUTPUT_PWD} ${textDecoder.decode(pwdBuffer)}`);
